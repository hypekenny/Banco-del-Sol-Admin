  ...
  allTransactions.forEach((tran) => {
    ...
    if (!monthlyData.length)
      monthlyData.push({
        ...
        name: tranMonth,
        dailyData: [
          { number: tranDayNumber, amount: tran.value, name: tranDay },
        ],
      });
    for (let i = 0; i < monthlyData.length; i++) {
      if (monthlyData[i].name === tranMonth) {
        for (let j = 0; j < monthlyData[i].dailyData.length; j++) {
          if (monthlyData[i].dailyData[j].number === tranDayNumber)
            monthlyData[i].dailyData[j].amount += tran.value;
          else
  ...
  return (
    <div className={styles.container}>
      <div className={styles.chart}>
        <h1 className={styles.h1Estadisticas}>Estadisticas Mensuales</h1>
      ...
      <div className={styles.chart}>
        <h1 className={styles.h1Estadisticas}>Estadisticas Diaria</h1>
